...
export default function DirectoryPage() {
  ...
  const handleStartChat = (colleague: any) => {
    // Navigate to Chat, passing details in query params for auto-selection
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Explore skills, locate office departments, and connect with teammates instantly.</p>
      </div>

      {/* Directory Summary Metrics */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-left">
        {[
          { label: 'Total Teammates', value: data?.users?.length || 0, icon: '👥', color: 'bg-blue-50/50 dark:bg-blue-950/15 text-blue-600 border-blue-100 dark:border-blue-900/30' },
          { label: 'Online Now', value: data?.users?.filter((u: any) => u.status === 'ONLINE').length || 0, icon: '🟢', color: 'bg-green-50/50 dark:bg-green-950/15 text-green-600 border-green-100 dark:border-green-900/30' },
          { label: 'Active Depts', value: Array.from(new Set(data?.users?.map((u: any) => u.department?.name).filter(Boolean))).length || 0, icon: '🏢', color: 'bg-primary/5 text-primary border-primary/10' },
          { label: 'Office Locations', value: Array.from(new Set(data?.users?.map((u: any) => u.location).filter(Boolean))).length || 0, icon: '📍', color: 'bg-amber-50/50 dark:bg-amber-950/15 text-amber-500 border-amber-100 dark:border-amber-900/30' }
        ].map((card, idx) => (
      ...
      <div className="bg-white dark:bg-slate-900 border p-4 rounded-2xl shadow-sm grid grid-cols-1 md:grid-cols-4 gap-4 font-semibold text-xs text-muted-foreground items-end">
        <div className="space-y-1 text-left">
          <label className="uppercase text-[9px] tracking-wider">Search Name or Skill</label>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
            <input
              type="text"
              placeholder="e.g. Alice or React..."
              value={search}
        ...
        <div className="space-y-1 text-left">
          <label className="uppercase text-[9px] tracking-wider">Office Location</label>
          <input
            type="text"
        ...
        <div className="space-y-1 text-left">
          <label className="uppercase text-[9px] tracking-wider">Presence Status</label>
          <select
            value={status}
              ...
              <div className="space-y-3.5">
                ...
                <div className="space-y-1.5 text-[10px] text-slate-505 dark:text-slate-400 font-semibold leading-none text-left">
                  <div className="flex items-center space-x-1.5">
                    ...
                    <span>{colleague.location || 'Remote'}</span>
                  </div>
                  <div className="flex items-center space-x-1.5">
            ...
            <tbody>
              {data?.users?.map((colleague: any) => (
                ...
                >
                  ...
                  <td className="p-4 text-slate-505 font-medium">{colleague.department?.name || 'General Org'}</td>
                  <td className="p-4 text-slate-505 font-medium">{colleague.location || 'Remote'}</td>
                  <td className="p-4">
          ...
          <div className="bg-white dark:bg-slate-900 border p-6 rounded-3xl w-full max-w-md space-y-6 shadow-2xl relative">
            ...
            {selectedUser.bio && (
              <div className="space-y-1 text-xs">
                ...
                <p className="p-3 bg-slate-50 dark:bg-slate-800/40 border rounded-xl text-slate-600 dark:text-slate-350 leading-relaxed">
                  {selectedUser.bio}
                </p>
              </div>
            )}
